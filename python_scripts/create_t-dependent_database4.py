...
for t in range(num_states):
    ...
    for j in range(num_points_y):
        ...
        for i in range(num_points_x):
            ...
            # Modify the z-coordinate calculation to create two opposite spirals on the same XY plane with equal amplitude
            r1 = np.sqrt(x ** 2 + y ** 2)  # Point 1 at (0, 0)
            theta1 = np.arctan2(y, x)  # Calculate angle with respect to x-axis for point 1
            z1 = amplitude * np.sin(r1 - t / smoothness_variable + theta1)  # Add angles to z-coordinate for point 1 with amplitude

            r2 = np.sqrt((x - black_hole_radius) ** 2 + y ** 2)  # Point 2 at (-black_hole_radius, 0)
            theta2 = np.arctan2(y, x - black_hole_radius)  # Calculate angle with respect to x-axis for point 2
            z2 = amplitude * np.sin(r2 - t / smoothness_variable + theta2)  # Add angles to z-coordinate for point 2 with amplitude

            # Set the z-coordinates of both points to be the same, so that they are on the same XY plane
            z = (z1 + z2) / 2
